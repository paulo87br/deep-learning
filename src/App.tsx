...
import { ArrowRight, ChevronLeft, ChevronRight, Maximize, Moon, Pause, Play, Radio, RotateCcw, Send, Sun, X } from 'lucide-react'
...
import { ClassroomBus, sanitizeRoom, type ConnectionState } from './supabase'
...
function DisplayPage({ room }: { room: string }) {
  ...
  const [showLearning, setShowLearning] = useState(false); const [selectedFeature, setSelectedFeature] = useState(0); const scenarioRef = useRef(scenario); const predictionSentRef = useRef(true)
  const bus = useMemo(() => new ClassroomBus(room, 'display'), [room])
  const focusPath = useMemo(() => classroomNetwork.influencePath(scenario, selectedFeature), [scenario, selectedFeature, trace])
  const focusTrail = [FEATURES[selectedFeature].label, LAYER_LABELS[1][focusPath[1]], LAYER_LABELS[2][focusPath[2]], LAYER_LABELS[3][focusPath[3]]].join(' → ')
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      if (event.key === 'ArrowLeft') { setStage((current) => Math.max(0, current - 1)); setPlaying(false) }
      if (event.key === 'ArrowRight') { setStage((current) => Math.min(7, current + 1)); setPlaying(false) }
      if (event.key.toLocaleLowerCase('pt-BR') === 'r') setResetId((id) => id + 1)
      ...
      if (event.key === 'Escape') setShowLearning(false)
    }
  ...
  return <main className={`display-page ${theme}`}><header><div><Brand/><span className="eyebrow">Deep Learning · Sala {room}</span><h1>Como uma máquina aprende com um erro?</h1></div><div className="header-actions"><a className="tablet-link" href={`/input?room=${room}`} target="_blank" rel="noreferrer"><Send size={15}/>Tela do participante</a><Connection state={connection}/><button className="icon-button" onClick={toggleTheme} aria-label={theme === 'dark' ? 'Ativar tema claro' : 'Ativar tema escuro'}>{theme === 'dark' ? <Sun/> : <Moon/>}</button><button className="icon-button" onClick={()=>setResetId((id)=>id+1)} aria-label="Restaurar câmera"><RotateCcw/></button><button className="icon-button" onClick={()=>void document.documentElement.requestFullscreen()} aria-label="Entrar em tela cheia"><Maximize/></button></div></header><section className="display-grid"><aside className="scenario-panel"><span className="panel-label">Situação do participante</span><small className="focus-label">Escolha um fato para acompanhar pela rede</small>{FEATURES.map((feature, featureIndex) => { const value = scenario[feature.key as keyof Scenario]; return <button type="button" className={`scenario-value ${featureIndex === selectedFeature ? 'selected' : ''}`} aria-pressed={featureIndex === selectedFeature} onClick={() => setSelectedFeature(featureIndex)} key={feature.key}><span>{feature.label}</span><strong>{feature.key === 'transit' ? value ? 'Sim' : 'Não' : `${value}${feature.unit}`}</strong><i><b style={{width:`${feature.key === 'distance' ? value/30*100 : feature.key === 'transit' ? value*100 : value}%`}}/></i></button> })}<button className="button secondary" onClick={()=>void reset()}><RotateCcw/>Nova rodada</button></aside><section className="visual-stage"><nav className="stage-nav" aria-label="Etapas da rede">{STAGES.map((item,index)=><button key={item.short} className={index===stage?'active':index<stage?'done':''} aria-current={index===stage?'step':undefined} aria-label={`Etapa ${index+1}: ${item.short}`} onClick={()=>{setStage(index);setPlaying(false)}}><i>{index+1}</i><span>{item.short}</span></button>)}</nav><NetworkScene trace={trace} activeStage={stage} theme={theme} resetId={resetId} focusPath={focusPath} onSelectLayer={(layer)=>{setStage(Math.min(7,layer*2));setPlaying(false)}}/><div className="network-layer-map"><span>Camada 1<strong>Entrada · [6]</strong></span><span>Camada 2<strong>Oculta 1 · [8]</strong></span><span>Camada 3<strong>Oculta 2 · [6]</strong></span><span>Camada 4<strong>Saída · [4]</strong></span></div><small className="scene-help">Arraste para girar · Roda ou pinça para aproximar · Clique em um neurônio</small></section><aside className="explanation-panel" aria-live="polite"><span className="step-number">0{stage+1}</span><span className="eyebrow">{STAGES[stage].technical}</span><h2>{STAGES[stage].title}</h2><p>{STAGES[stage].text}</p><div className="calculation-heading"><span>Cálculo desta etapa</span><strong>Camada {STAGE_LAYERS[stage].number}/4 · {STAGE_LAYERS[stage].name}</strong><small>{STAGE_LAYERS[stage].shape}</small></div><div className="focus-trail"><small>Sinal acompanhado</small><strong>{focusTrail}</strong></div><Calculation scenario={scenario} trace={trace} stage={stage}/></aside></section><footer><button className="icon-button" aria-label="Etapa anterior" disabled={stage===0} onClick={()=>{setStage((s)=>Math.max(0,s-1));setPlaying(false)}}><ChevronLeft/></button><button className="button secondary" onClick={()=>setPlaying((value)=>!value)}>{playing?<><Pause/>Pausar</>:<><Play/>Reproduzir</>}</button><button className="icon-button" aria-label="Próxima etapa" disabled={stage===7} onClick={()=>{setStage((s)=>Math.min(7,s+1));setPlaying(false)}}><ChevronRight/></button>{learning && <button className="button primary" onClick={()=>setShowLearning(true)}>Ver como aprendeu</button>}</footer>{learning && showLearning && <LearningModal learning={learning} onClose={()=>setShowLearning(false)}/>}</main>
}
...
export default function App() {
  ...
  return <AuthGate brand={<Brand />} labName="Deep Learning ao Vivo">{path.startsWith('/input')?<InputPage room={room}/>:path.startsWith('/display')?<DisplayPage room={room}/>:<Home room={room} setRoom={setRoom}/>}</AuthGate>
}
