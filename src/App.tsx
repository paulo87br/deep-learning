...
function Calculation({ scenario, trace, stage }: { scenario: Scenario; trace: Trace; stage: number }) {
  ...
  if (stage === 5) return <div className="calc"><small>Notas antes dos percentuais</small>{CHOICES.map((c,i)=><code key={c.id}>{c.label}: {trace.logits[i].toFixed(2)}</code>)}</div>
  if (stage === 6) return <><div className="calc"><code>e^nota ÷ soma de todas as e^notas</code><p>Assim, todos os resultados passam a somar 100%.</p></div><ProbabilityBars trace={trace}/></>
  return <div className="decision"><b>{CHOICES.find(c=>c.id===trace.predicted)?.icon}</b><span>A rede prevê</span><strong>{choiceLabel(trace.predicted)}</strong><p>Isso é uma hipótese calculada, não uma certeza.</p></div>
}
...
function DisplayPage({ room }: { room: string }) {
  ...
  return <main className={`display-page ${theme}`}><header><div><Brand/><span className="eyebrow">Deep Learning · Sala {room}</span><h1>Como uma máquina aprende com um erro?</h1></div><div className="header-actions"><Connection state={connection}/><button className="icon-button" onClick={toggleTheme}>{theme === 'dark' ? <Sun/> : <Moon/>}</button><button className="icon-button" onClick={()=>setResetId((id)=>id+1)}><RotateCcw/></button><button className="icon-button" onClick={()=>void document.documentElement.requestFullscreen()}><Maximize/></button></div></header><section className="display-grid"><aside className="scenario-panel"><span className="panel-label">Situação do participante</span>{FEATURES.map((feature) => { const value = scenario[feature.key as keyof Scenario]; return <div className="scenario-value" key={feature.key}><span>{feature.label}</span><strong>{feature.key === 'transit' ? value ? 'Sim' : 'Não' : `${value}${feature.unit}`}</strong><i><b style={{width:`${feature.key === 'distance' ? value/30*100 : feature.key === 'transit' ? value*100 : value}%`}}/></i></div> })}<button className="button secondary" onClick={()=>void reset()}><RotateCcw/>Nova rodada</button></aside><section className="visual-stage"><nav className="stage-nav">{STAGES.map((item,index)=><button key={item.short} className={index===stage?'active':index<stage?'done':''} onClick={()=>{setStage(index);setPlaying(false)}}><i>{index+1}</i><span>{item.short}</span></button>)}</nav><NetworkScene trace={trace} activeStage={stage} theme={theme} resetId={resetId} onSelectLayer={(layer)=>{setStage(Math.min(7,layer*2));setPlaying(false)}}/><small className="scene-help">Arraste para girar · Roda ou pinça para aproximar · Clique em um neurônio</small></section><aside className="explanation-panel"><span className="step-number">0{stage+1}</span><span className="eyebrow">{STAGES[stage].technical}</span><h2>{STAGES[stage].title}</h2><p>{STAGES[stage].text}</p><Calculation scenario={scenario} trace={trace} stage={stage}/></aside></section><footer><button className="icon-button" disabled={stage===0} onClick={()=>{setStage((s)=>Math.max(0,s-1));setPlaying(false)}}><ChevronLeft/></button><button className="button secondary" onClick={()=>setPlaying((value)=>!value)}>{playing?<><Pause/>Pausar</>:<><Play/>Reproduzir</>}</button><button className="icon-button" disabled={stage===7} onClick={()=>{setStage((s)=>Math.min(7,s+1));setPlaying(false)}}><ChevronRight/></button>{learning && <button className="button primary" onClick={()=>setShowLearning(true)}>Ver como aprendeu</button>}</footer>{learning && showLearning && <LearningModal learning={learning} onClose={()=>setShowLearning(false)}/>}</main>
}
