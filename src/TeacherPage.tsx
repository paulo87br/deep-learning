...
export default function TeacherPage({ room }: { room: string }) {
  ...
  return <main className="teacher-page"><header><Brand/><a href="/">Voltar ao início</a></header><section className="teacher-hero"><div><span className="eyebrow">Painel do professor · Sala {room}</span><h1>Prepare a aula<br/><em>em duas telas.</em></h1><p>Projete a rede neural e compartilhe o QR Code com os participantes. Todos entram na mesma sala automaticamente.</p></div><div className="teacher-qr"><span>Entrada dos participantes</span>{qr ? <img src={qr} alt={`QR Code para entrar na sala ${room}`}/> : <div className="qr-placeholder"/>}<strong>{room}</strong><small>Aponte a câmera do tablet</small></div></section><section className="teacher-links"><LinkCard label="Tela do projetor" description="Abra no computador conectado à projeção" href={displayUrl} icon={<Monitor/>}/><LinkCard label="Tela do participante" description="Compartilhe no tablet ou pelo QR Code" href={participantUrl} icon={<Send/>}/></section></main>
}
